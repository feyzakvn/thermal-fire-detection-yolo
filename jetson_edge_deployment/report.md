...
This directory houses the **actual, empirical hardware execution assets** generated during the live deployment of our fire detection model on the target edge computer. 
...
5. **`bitmap/`**: A dedicated subdirectory containing 17 sequential radiometric heatmaps captured during the laboratory tests. It includes **`bitmap_162105_CRITICAL.png`**, the empirical evidence saved instantly by the pipeline during the live combustion (lighter flame) verification test.

---

### 🖼️ Live Laboratory Validation Proof
Below is the instant hardware snapshot (`bitmap/bitmap_162105_CRITICAL.png`) extracted during the live lighter ignition test. The red core pinpoints the exact structural location of the flame exceeding 60°C, perfectly cross-referenced with the spatial depth metrics.
...
The system cross-references the YOLOv8 visual inference with the raw matrices from the Flir Lepton and Intel RealSense cameras using the following constraints:
